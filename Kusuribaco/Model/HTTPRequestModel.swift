...
class HTTPRequest{
                ...
                if let result = String(data: data!, encoding: .japaneseEUC) {
                    if let doc = try? Kanna.HTML(html: result, encoding: String.Encoding.utf8) {
                        ...
                        for node in doc.xpath("//td[@colspan='3']/a[contains(@href, '/ogo/')]") {
                            ...
                            // ブックマークフラグ
                            if Bookmark.selectByName(drugName: drugString).count > 0 {
                                // ブックマーク済みだったら "1"
    ...
    class func getPdfURL(requestData: PdfRequestData, completion: @escaping (String) -> Void){

        var pdfString: String = ""

        // URL
        let url: URL = URL(string: requestData.url)!
        var request: URLRequest = URLRequest(url: url)
